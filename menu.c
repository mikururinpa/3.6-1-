...
void Show_Main_Menu(void) {
    ...
    OLED_ShowString(8, 16, (uint8_t*)"Run Mode", 16);
    OLED_ShowString(8, 32, (uint8_t*)"Settings", 16);
    OLED_ShowString(0, 48, (uint8_t*)"                ", 16);
    
    OLED_ShowString(0, 16, (uint8_t*)" ", 16);
    OLED_ShowString(0, 32, (uint8_t*)" ", 16);
    OLED_ShowString(0, 16 + cursor_main * 16, (uint8_t*)">", 16);
}
        ...
        switch (current_state) {
            case MENU_MAIN:
                if (key == 1) { cursor_main++; if (cursor_main > 1) cursor_main = 0; Show_Main_Menu(); }
                    ...
                    if (cursor_main == 0) { run_flag = 1; OLED_Clear(); } 
                    else if (cursor_main == 1) { current_state = MENU_SETTINGS; cursor_settings = 0; scroll_offset = 0; OLED_Clear(); Show_Settings_Menu(); }
                }
                break;
            case MENU_SETTINGS:
                ...
                else if (key == 4) { current_state = MENU_MAIN; OLED_Clear(); Show_Main_Menu(); }
                break;
            case MENU_EDIT_INT:
                if (key == 1) { 
